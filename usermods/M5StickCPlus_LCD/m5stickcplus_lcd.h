...
class m5stickcplus_lcd : public Usermod
{
private:
...
public:
  ...
  void setup()
  {
    ...
    delay(500);
    // Lcd display
    M5.Lcd.setRotation(1); // Rotation here is set up for the text to be readable with the port on the left. Use 1 to flip.
  ...
  void loop()
  {
    ...
    // Adjust Brightness of display after 5 minutes with no change.
    if (!displayTurnedOff && millis() - lastRedraw > 5 * 60 * 1000)
    {
      M5.Axp.ScreenBreath(lowBreath); // Turn backlight low.
    ...
    if (displayTurnedOff)
    {
      M5.Axp.ScreenBreath(highBreath); // Turn backlight high.
    ...
    M5.Lcd.print(knownSsid.substring(0, tftcharwidth > 1 ? tftcharwidth - 1 : 0));
    ...
    M5.Lcd.setTextSize(2);
    M5.Lcd.setCursor(1, 24);
    // Print AP IP and password in AP mode or knownIP if AP not active.
    // if (apActive && bri == 0)
    //   M5.Lcd.print(apPass);
    // else
    //   M5.Lcd.print(knownIp);

    if (apActive)
    {
      M5.Lcd.print("AP IP: ");
      M5.Lcd.print(knownIp);
      M5.Lcd.setCursor(1, 46);
      M5.Lcd.print("AP Pass:");
      M5.Lcd.print(apPass);
    }
    else
    {
    ...
    M5.Lcd.setCursor(1, 112);
    // Print estimated milliamp usage (must specify the LED type in LED prefs for this to be a reasonable estimate).
    M5.Lcd.print(strip.currentMilliamps);
    M5.Lcd.print("mA (estimated)");
  }
